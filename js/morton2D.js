...
yabaGameProto0.mortonSpaceObjectModel.morton2D = ( function() {
	const util = yabaGameProto0.util,
	      {STANDARD_LENGTH, QUAD, NUMBER_OF_CELLS, UNIT_LENGTH} = yabaGameProto0.config.getConfig();

	const NUMBER_OF_BITS = Math.log2(NUMBER_OF_CELLS),					// NUMBER_OF_CELLSのビット数
	      UNIT_SHIFT = 2,												// 2ビット(4通り)毎のシフト
	      MASK_FOR_SPACE_LEVEL = 0b11 << NUMBER_OF_BITS - UNIT_SHIFT;	// オブジェクトの所属空間レベルを求めるのに使う
	...
	const getLinerQuarternaryTreeIndex = (left, top, width, height) => {
		...
		const leftTopMortonNumber = to2DMortonNumber(left, top),
		      rightBottomMortonNumber = to2DMortonNumber(left + width, top + height);

		return	toLinerQuarternaryTreeIndex(leftTopMortonNumber, rightBottomMortonNumber);
	},

	toLinerQuarternaryTreeIndex = (leftTopMortonNumber, rightBottomMortonNumber) => {
		...
		const spaceLevel = i / UNIT_SHIFT,
		      indexOnSpaceLevel = leftTopMortonNumber >> NUMBER_OF_BITS - i;

		return util.calcNumberOfTermsTo(spaceLevel, QUAD) + indexOnSpaceLevel;

	},

	to2DMortonNumber = (x, y) => {	// 座標からモートン順序を求める
	...
	return {
		...
		toLinerQuarternaryTreeIndex: toLinerQuarternaryTreeIndex,
		to2DMortonNumber: to2DMortonNumber,
